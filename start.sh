...
demo_dataset_path="/user/au2056/tars/demo_dataset/raw"
hdfs dfs -mkdir $2/

echo $demo_dataset_path
...
hdfs dfs -cp -f "$demo_dataset_path/categories.csv" $2/clean/
hdfs dfs -cp -f "$demo_dataset_path/user-meta-data.csv" $2/clean/


#we'll be using the same jar file to execute all our spark jobs from here on.

#spark job to clean user data
spark-submit --conf spark.master=yarn --conf spark.yarn.maxAppAttempts=1 --class nyu.bdad.tars.UserDataCleaning tars_1.0.jar "$demo_dataset_path/raw-user-data.csv" "$2/clean/"

#spark job to clean video data
spark-submit --conf spark.master=yarn --class nyu.bdad.tars.VideoDataCleaning tars_1.0.jar "$demo_dataset_path/raw-video-data.csv" "$2/clean/"

echo "data cleaning done"

#spark job to generate watch history
spark-submit --conf spark.master=yarn --num-executors 10 --executor-cores 3 --executor-memory 3G --conf spark.yarn.maxAppAttempts=1 --class nyu.bdad.tars.WatchHistoryGeneration tars_1.0.jar "$2/clean/categories.csv" "$2/clean/user/" "$2/clean/video/" "$2/clean/user-meta-data.csv" "$2/wh/" ".05" ".05"
